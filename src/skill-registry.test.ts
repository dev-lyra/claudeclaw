...
Read and send emails.`,
    ...
    // Should find at least the global skill (project skills/ may also be loaded)
    const gmailSkill = all.find((s) => s.id === 'gmail');
    expect(gmailSkill).toBeDefined();
    // Name comes from frontmatter - may be overridden by project skills dir
    expect(gmailSkill!.triggerWords.length).toBeGreaterThan(0);
  });

  it('handles missing skill directories gracefully', () => {
    // Remove the temp global skills dir so only project skills remain
    ...
    // May still find project-level skills from the real skills/ dir
  });
});
...
---`,
  ...
  it('matches trigger words to skills', () => {
  ...
  it('returns empty array for unrelated messages', () => {
    const matches = matchSkills('xyzzy nonsense gibberish');
...
---`,
    ...
    // Should contain gmail skill entry (description may vary based on real SKILL.md)
  ...
  it('returns consistent format', () => {
    initSkillRegistry();
    const index = getSkillIndex();
    // Each line should be "id: description" format
    if (index.length > 0) {
...
Full instructions here.`;
    ...
    // Instructions come from whichever gmail skill was loaded (temp or project)
